...
export function renderMedia(item) {
    ...
    switch (type) {
        ...
        case 'image':
            return ` <img data-item-preiewe data-id="${id}" title="${title}" alt="title" src="${item.url}" loading="lazy" alt="${escapeHtml(title)}" >`;

        case 'video':
            return `<video data-item-preiew data-id="${id}" title="${title}" src="${item.url}" preload="metadata"></video>`;

        case 'audio':
            return `<audio data-item-preiew data-id="${id}" title="${title}" src="${item.url}" preload="metadata"></audio>`;
